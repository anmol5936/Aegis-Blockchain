...
const App: React.FC = () => {
  const {
    ...
    appNotifications, // Use notifications from context
    addAppNotification, // Use addNotification from context
    stakeInPool,
    unstakeFromPool,
    fetchUserData,
    fallbackPayWithCrossPools, // Updated function name
  ...
  // Local UI state
  const [pools, setPools] = useState<LiquidityPoolData[]>([]); // Will be populated from blockchain
  const [currentUserData, setCurrentUserData] = useState<UserAccount | null>(
    null
  ); // For data linked to connected wallet
  ...
  useEffect(() => {
    loadBlockchainPools();
  }, [loadBlockchainPools, address]); // Reload pools if address changes (e.g. factory is ready)
  ...
  const handleCreatePool = async (regionName: string) => {
    ...
    if (createPoolOnChain) {
      ...
      if (success) {
        // Re-fetch pools to update the list from the blockchain
  ...
  const handleStake = async (
    ...
    try {
      ...
      if (isUnstaking) {
        // For unstaking, we need to convert the staked amount to LP tokens
        ...
        // Calculate LP tokens to burn based on proportional amount
  ...
  const handleDistributeRewards = useCallback(async () => {
    ...
    // TODO: Call context function that iterates through pools and calls distributeRewards on each
    // const { distributeAllRewardsOnChain } = useStateContext();
    // await distributeAllRewardsOnChain();
    // After success: await loadBlockchainPools();
  ...
  const togglePoolStatus = async (poolId: string) => {
    ...
    // TODO: Implement actual smart contract call via context
    // const { togglePoolStatusOnChain } = useStateContext();
    // await togglePoolStatusOnChain(poolId);
    // After success: await loadBlockchainPools();
  };
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col">
      ...
      <div className="fixed top-20 right-5 z-50 space-y-2 w-full max-w-xs sm:max-w-sm">
        {appNotifications.map(
          (
            n: AppNotificationType // Use AppNotifications from context
          ) => (
            <div
              key={n.id}
          ...
          <div className="flex items-center gap-4">
            {address ? (
              ...
              <button
                ...
                disabled={isLoading}
                className="p-2.5 bg-sky-600 hover:bg-sky-700 rounded-md text-white flex items-center gap-2 text-sm disabled:bg-slate-600 disabled:cursor-not-allowed"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pools.map((pool) => (
            <PoolCard
              key={pool.id}
              pool={pool}
              currentUser={currentUserData} // Use blockchain-linked user data
